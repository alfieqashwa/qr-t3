...
type RegenciesProps = {
  regencies?: RouterOutputs["address"]["getRegencies"];
  isLoading: boolean;
};

export function RegenciesCombobox({ regencies, isLoading }: RegenciesProps) {
  const [open, setOpen] = React.useState(false);
  const [name, setName] = React.useState("");
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        ...
        >
          {name
            ? regencies?.find((regency) => regency.name === name.toUpperCase())
                ?.name
            : "Select regencies..."}
          <ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50" />
        </Button>
